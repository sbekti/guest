...
export const RegisterCard = ({ onRegisterSuccess }) => {
  ...
  const handleSubmit = (values, setSubmitting, resetForm, setErrors, setFieldValue) => {
      ...
      >
        <Stack spacing={6}>
          ...
          <Formik
            ...
            onSubmit={(values, { setSubmitting, resetForm, setErrors, setFieldValue }) => {
              handleSubmit(values, setSubmitting, resetForm, setErrors, setFieldValue);
            }}
          >
                      ...
                      <FormControl isInvalid={meta.touched && meta.error}>
                        <FormLabel w='100%'>
                          ...
                          <Box align='center' mt={1} mb={1}>
                            <Image h={20} w={60} src={captchaUrl} alt='CAPTCHA image' />
                          </Box>
                        </FormLabel>
                        <Input type='text' inputMode='numeric' {...field} />
                        <Input type='hidden' name='captchaId' defaultValue={captchaId} />
                        <ErrorMessage name='captchaAnswer' component={FormErrorMessage} />
                      </FormControl>
                    )}
                  </Field>
                  <Stack spacing={10}>
                    <Field type='checkbox' name='agreed'>
                      {() => (
                        <Checkbox isChecked={values.agreed} onChange={(e) => setFieldValue('agreed', e.target.checked)}>
                          I agree to the <Link color={'blue.400'} as={RouteLink} to='/terms'>Terms of Use</Link>
                        </Checkbox>
                      )}
                    </Field>
                    <Button
